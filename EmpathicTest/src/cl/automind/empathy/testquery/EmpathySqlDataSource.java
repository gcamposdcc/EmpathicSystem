...
public class EmpathySqlDataSource extends AbstractSqlDataSource<EmpathyMetadata>{
	...
	public Collection<SqlNamedValuePair<?>> toPairs(EmpathyMetadata target) {
		...
		pairs.add(Sql.pair("id", target.getId()));
		pairs.add(Sql.pair("idsource", target.getIdsource()));
		pairs.add(Sql.pair("idtype", target.getIdtype()));
		pairs.add(Sql.pair("evaluated", target.isEvaluated()));
		pairs.add(Sql.pair("answered", target.isAnswered()));
		pairs.add(Sql.pair("liked", target.isLiked()));
		pairs.add(Sql.pair("created_at", target.getCreatedAt()));
		return pairs;
	}
